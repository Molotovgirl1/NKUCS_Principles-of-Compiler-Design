...
private:
    std::vector<BasicBlock *> pred, succ;//用邻接链表来构建，后继可能出现超过1的情况，所以是vector
...
public:
    ...
    void output() const;
    bool succEmpty() const { return succ.empty(); };//vector自带
    ...
    Instruction* rbegin() { return head->getPrev();};//反向遍历指令列表
